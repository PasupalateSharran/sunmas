(function ($) {
	...
	document.addEventListener('DOMContentLoaded', function () {
		// Get references to the button and login container
		const openLoginButton = document.getElementById('openLoginContainer');
		...
		// Event listener to close the login container when clicking outside of it or on the body
		document.addEventListener('click', function (event) {
			const target = event.target;
			if (!loginContainer.contains(target) && target !== openLoginButton) {
				closeLogin();
			}
		});
	});
